@extends('templates.app', ['title' => 'Income || PEKU.ID'])

@section('dynamic-content')
    <div class="my-3">
        <a href="{{ route('income.add') }}" class="btn btn-success mb-3">+ Tambah</a>
            ...
            <tbody>
                @if (count($incomes) > 0)
                    @foreach ($incomes as $index => $item)
                        <tr>
                            <td>{{ ($incomes->currentPage() - 1) * $incomes->perpage() + ($index + 1) }}</td>
                            <td>{{ $item['name'] }}</td>
                            <td>{{ 'Rp ' . number_format($item['jumlah'], 0, ',', '.') }}</td> <!-- Format rupiah -->
                            <td>{{ $item['jenis'] }}</td>
            
                            <td class="d-flex justify-content-center py-1">
                                <a href="{{ route('income.edit', $item['id']) }}" class="btn btn-primary me-3">Edit</a>
